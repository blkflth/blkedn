...
Nothing super complex as far as NixOS settings go.
...
- If CoolerControl isn't able to see your system fans, run ``lm_sensors`` and follow the on-screen instructions. Thereafter, writte the indicated kernel module's name  into the ``boot.kernelModules`` field in ``configuration.nix`` and reboot.
